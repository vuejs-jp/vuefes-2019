...
export default defineComponent({
  ...
  methods: {
    staffAvatarSrc(avatar: string): string {
      return getAssetImage(`staffs/${avatar}`)
    },
    staffAvatarSrcSet(avatar: string): string {
      return createSrcSet(`staffs/${avatar}`, `staffs/@2x/${avatar}`)
    },
    volunteerAvatarSrc(avatar: string): string {
      return getAssetImage(`volunteers/${avatar}`)
    },
    volunteerAvatarSrcSet(avatar: string): string {
      return createSrcSet(`volunteers/${avatar}`, `volunteers/@2x/${avatar}`)
    },
    compareStaffNames(a: string, b: string): number {
      const adjustedA = a.toLowerCase().replace(/_/g, '')
      const adjustedB = b.toLowerCase().replace(/_/g, '')
