...
export const state = (): Sessions.State => ({
  sessions: [
    ...
    },
    {
      ...
      time: 40,
      paragraphs: [
        'Retty は、月間 4000 万人のユニークユーザーを持つ実名型グルメサービスです。9年間運用されてきましたが、当時の JavaScript はフル jQuery で書かれていました。本セッションでは、ここ数年で実施したフロントエンドの改善、サービス運用と並行しながら部分的に Vue.js、Nuxt.js に移行した軌跡をお話しします。',
